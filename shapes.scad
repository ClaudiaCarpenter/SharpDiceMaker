...
module deltohedron(height, angle) {
	...
	intersection() {
		...
		if (cut_corners)
			cylinder(d = cut_modifier * height, h = 1.38 * height, center = true);
	}
}
		...
		scale([ height, height, height ]) {
			polyhedron(points =
			               [
				               ...
				               [ -0.288675, -0.5, /* -0.27217 */ -0.20417 ], [ 0.57735, 0, /* -0.27217 */ -0.20417 ],
				               [ 0, 0, /* 0.54432548 */ 0.612325 ]
			               ],
			           faces = [ [ 1, 2, 0 ], [ 3, 2, 1 ], [ 3, 1, 0 ], [ 2, 3, 0 ] ]);
		}
	}
}
...
module crystal(face_edge, body_length, end_height, makeHollow = false, add_sprue, sprue_diameter = 2, sprue_angle = 0) {
    ...
    // if (cut_corners) {
    //   echo("corners")
    //   cube([face_edge, face_edge, face_edge * 1.65])
    // }
	//}
}

module trigonal_trapezohedron(face_edge, angle = 33, makeHollow = false, add_sprue, sprue_diameter = 2,
                              sprue_angle = 0) {
...
module make_sprue_hole(x_offset, y_offset, z_offset, diameter, angle = 0, height = 8) {
	...
	color("blue") translate([ x_offset, y_offset, z_offset ])
	rotate([ angle, 0, 0 ])
...
module generate_sprue(hole_diameter) {
	...
	scale([ 0.95, 0.95, 1 ]) { // wiggle room
		...
		cylinder(h = height_tip, r1 = hole_diameter * 0.5, r2 = hole_diameter * 0.5, center = false,
		         $fn = hole_diameter * 25);
...
module render_base(length, height, width, support_offset = 0) {
	...
	points = [ [ 0, 0 ], [ 0, base_height ], [ base_length, base_height ], [ base_length, 0 ] ];

	color(support_color) rotate([ 90, 0, 0 ])
	linear_extrude(height = size, center = true) polygon(points = points);
}
...
module render_raft(length, support_offset, num) {
	...
	color(support_color) for (i = [0:num]) rotate([ 0, 0, i * rotate_by ])
